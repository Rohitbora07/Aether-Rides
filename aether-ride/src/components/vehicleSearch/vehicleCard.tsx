...
import { Bike, Car, Clock, Gauge, IndianRupee, Star, Truck,ArrowRight } from "lucide-react"
...
function vehicleCard({ vehicle, distance, onBook }: VehicleCardProps) {
    const { Icon, label } = TYPE_CONFIG[vehicle.type]
    let estimatedFare: number = 0
    if (distance && vehicle.baseFare && vehicle.pricePerKm) {
        estimatedFare = Math.round(vehicle.baseFare + vehicle.pricePerKm * distance)
    }
    return (
        <motion.div
            initial={{ opacity: 0, y: 20 , backgroundColor: "#ffffff" }}
            animate={{ opacity: 1, y: 0 }}
            whileHover={{ scale: 1.01, backgroundColor: "#D9EAF7", borderColor: "#111111" }}
        ...
        >
            <div className="relative h-48 bg-zinc-50 group-hover:bg-[#D9EAF7] transition-colors duration-[0.28] flex items-center justify-center overflow-hidden">
                <div
                ...
                <div className='absolute bottom-3 left-3 z-20 flex items-center gap-1 bg-white border border-zinc-200 Dtext-zinc-700 text-[10px] font-bold px-2.5 py-1.5 rounded-full shadow-sm"'>
                    <Star size={9} className="Ofill-zinc-900 Otext-zinc-900" />4.8
                </div>
            </div>

            <div className="h-px bg-black w-auto ml-7 mr-8 " />
                ...
                <div className="grid grid-cols-2 gap-2">
                    <div className="bg-zinc-50 border border-zinc-100 rounded-2xl px-3.5 py-3">
                        ...
                        <p className="text-zinc-900 text-sm flex items-center font-black">
                            <IndianRupee size={11} />{vehicle.pricePerKm}</p>
                    </div>
                    ...
                    <div className="bg-zinc-50 border border-zinc-100 rounded-2xl px-3.5 py-3">
                        ...
                        <div className="text-zinc-900 text-sm font-black flex items-center">
                            <IndianRupee size={11} />
                            {vehicle.waitingChargePerMin}
                            <span>/min</span>
                        </div>
                    </div>
                </div>

                <div className="flex items-end justify-between pt-3 border-t border-black">
                    <div>
                        ...
                        >
                            ...
                            />
                            <span className="text-zinc-900 text-3xl font-black tracking-tight leading-none">{estimatedFare}</span>
                        </motion.div>
                    </div>
